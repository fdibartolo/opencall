source 'https://rubygems.org'
ruby '2.2.1'

gem 'rails', '4.2.7.1'
gem 'puma', '3.10.0'
gem 'pg', '0.21.0'

gem 'jbuilder', '2.7.0'
gem 'sass-rails', '5.0.6'
gem 'bootstrap', '4.0.0.alpha3' # watchout for breaking changes on .alpha5
gem 'rails-assets-tether', '1.1.1'
gem 'font-awesome-rails', '4.7.0.2'
gem 'uglifier', '3.2.0'
gem 'coffee-rails', '4.2.2'
gem 'angularjs-rails', '1.6.2'
gem 'jquery-rails', '4.3.1'
gem 'animate-rails', '1.0.10'
gem 'twitter-typeahead-rails', '0.11.1'
gem 'marked-rails', '0.3.2.0'
gem 'momentjs-rails', '2.17.1'
gem 'ng-toaster-rails', '0.4.15.0'
gem 'ng-sticky-rails', '1.7.9.0'
gem 'highcharts-rails', '4.2.5'
gem 'highcharts-ng-rails', '0.0.7'

gem 'devise', '4.3.0'
gem 'omniauth-oauth2', '1.3.1' # upgrading to 1.4.0 injects a defect on linkedin oauth
gem 'omniauth-github', '1.1.2'
gem 'omniauth-google-oauth2', '0.3.1'
gem 'omniauth-linkedin-oauth2', '0.1.5'
gem 'cancancan', '2.0.0'
gem 'twitter', '5.17.0'

gem 'will_paginate', '3.1.6'
gem 'elasticsearch-model', '0.1.9'
gem 'elasticsearch-rails', '0.1.9'
gem 'elasticsearch-extensions', '0.0.20' # 0.0.22 introduces a refactor that makes Travis to fail rspecs: https://github.com/elastic/elasticsearch-ruby/issues/342

gem 'newrelic_rpm', '4.4.0.336'
gem 'workflow', '1.2.0'

group :development, :test do
  gem 'spring', '2.0.2'
  gem 'spring-commands-rspec', '1.0.4'
  gem 'rspec-rails', '3.6.1'
  gem 'factory_girl_rails', '4.8.0'
  gem 'protractor-rails', '0.0.19'
  gem 'coveralls', '0.8.9', require: false
  gem 'letter_opener', '1.4.1'
end

gem 'rails_12factor', '0.0.3', group: :production

gem 'sdoc', '0.4.2', group: :doc
